...
class PNGLoader
{
	...
	void handlePixel(const SColor &in pixel, int offset)
	{
		...
		else
		{
            ...
            case map_colors::ship_spawner:          autotile(offset); spawnBlob(map, "ship_spawner", offset);           break;
            case map_colors::ship_map_picker:       autotile(offset); spawnBlob(map, "ship_map_picker", offset);        break;
...
CBlob@ spawnBlob(CMap@ map, const string &in name, u8 team, Vec2f position, s16 angle, const bool fixed)
{
    ...
    if(shape != @null)
    {
        shape.SetStatic(fixed);
    }
    else
    {
        error("warning, shape not found. likely failed to spawn blob.");
    }
